            ...
            <div key={index} className="flex flex-col gap-3">
              <div className="flex justify-center">
                <Link to={`/rackets/${racket.id}`} key={index}>
                  <img src={racket.product_image_1} alt={racket.product_name} className="w-60" />
                </Link>
              </div>
              ...
              <div className='flex flex-row justify-center items-center w-full'>
                <div className="w-1/2">
                  <input type="number" value={quantities[racket.id] || 1} onChange={(e) => setQuantities({ ...quantities, [racket.id]: Number(e.target.value) })} className="border border-gray-200 rounded-xl text-center text-md text-gray-400 px-5 w-full focus:outline-none focus:border-gray-400" />
                </div>
                <div className="w-1/2">
                  <button onClick={() => addToCart(racket.id, quantities[racket.id] || 1)} className="border rounded-xl text-center text-white text-xs py-1 px-5 bg-[#222] hover:bg-gray-800 cursor-pointer">Add to Cart</button>
                </div>
              </div>
            </div>
          )
